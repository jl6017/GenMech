...
COLINEAR = 0.001  # threshold for collinearity check
...
def input_base_numbers_random(margin=TRI_MARGIN):
    """
    Generate the base input link lengths, ensuring they differ by at least a margin.
    """
    while True:
        input_link, base_link = np.random.uniform(MIN_LENGTH, MAX_LENGTH, size=2)
        if abs(input_link - base_link) > margin:
            return np.array([input_link, base_link])
...
class mechanism_slider:
    def __init__(self, num_triangles=2, mech_id=0, pixel=224, filter=True):
        """
        ...
        self.num_points = num_triangles + 3  # slider mechanism has two fixed points and one coupler point
        self.sim_steps = 100
        self.pixel = pixel
        ...
        # the first 4 rows of links are different for slider mechanism
        self.links[:4] = np.array([
            [0, 1],  # Input link
            [0, 2],  # Base link
            [0, 3],  # slider
            [1, 3],  # coupler
        ...
        if filter:
            while True:
                ...
                self.normalize()
                if not self.collinear_check_slider(threshold=COLINEAR) and self.curve_smooth_check(threshold=CURVE_SMOOTH) and self.curve_range_check(threshold=CURVE_SIZE):
                    break
                else:
    ...
    def generate_mechanism(self):
        ...
        # input and base
        self.link_lengths[:2] = input_base_numbers_random()
        if self.link_lengths[0] > self.link_lengths[1]:
            self.cr = False  # if input link is longer than base link, it is not a crank-rocker mechanism
        else:
            self.cr = True
        ...
        self.points[:4] = [
            origin + np.array([-self.link_lengths[1], 0.0], dtype=np.float32),
            origin + np.array([-self.link_lengths[1], self.link_lengths[0]], dtype=np.float32),
            origin + np.array([self.link_lengths[1], 0.0], dtype=np.float32),
            origin
        ]

        # slider, link [0, 3] and [1, 3]:
        self.link_lengths[2] = self.link_lengths[1]  # slider link length
        self.link_lengths[3] = np.sqrt(np.sum((self.points[1] - self.points[3]) ** 2)) # Coupler link length
        # triangles
        for tri_num in range(2, self.num_triangles + 1):
    ...
    def simulate(self, tri_num):
        ...
        for step in range(1, self.sim_steps):
            ...
            # slider point solver
            x = np.sqrt(self.link_lengths[3] ** 2 - rotated_vector[1] ** 2)  # x-coordinate of the slider point
            self.path[step, 3] = self.path[0, 0] + [rotated_vector[0] + x, 0]  # Coupler point, x-coordinate is adjusted to be on the right side of the input link

            # Use triangle solver to find other points
            for tri in range(2, tri_num + 1):
    ...
    def collinear_check_slider(self, threshold):
        """
        ...
        joints = [x for x in range(self.num_points) if x != 2] # Exclude the slider point (index 3)

        for i, j, k in combinations(joints, 3):
            ## for slider, skip point 2
    ...
    def image(self, line_width=3, time_step=0, draw_coupler=True, draw_mechanism=True, speed_color=True):
        ...
        if draw_mechanism:
            ...
            for i in range(self.num_points):
                if i == 3:  # slider point, square
                    cv2.rectangle(img, 
                                  (int(self.path[time_step][i][0] * scale + padding) - 6, int((1 - self.path[time_step][i][1]) * scale + padding) - 6),
                                  (int(self.path[time_step][i][0] * scale + padding) + 6, int((1 - self.path[time_step][i][1]) * scale + padding) + 6),
                                  (255, 255, 0), -1)
                    
                else:  # other points, circle
...
def data_collection(dataset_size=100, 
                    num_triangles=2,
                    mechanism_id=0,
                    output_dir='stri_2/',
                    video=False
    ...
    for i in tqdm(range(dataset_size), desc=f"Data {output_dir}"):
        mech = mechanism_slider(num_triangles=num_triangles, mech_id=mechanism_id, pixel=224, filter=True)
...
def complex_dataset(graph_dict):
    for num_triangles in range(2, 6):
        graph_list = graph_dict[f"tri_{num_triangles}"]
        for iso_graph in graph_list:
            data_collection(dataset_size=200, 
                            num_triangles=num_triangles,
                            mechanism_id=int(iso_graph["id"].split('_')[-1]),
                            output_dir=f'complex_large/s{iso_graph["id"]}/'
                            )
            

if __name__ == "__main__":
    # mech = mechanism_slider(pixel=448)
    ...
    # data collection
    # data_collection(dataset_size=10000, output_dir='test/stri_2/', video=False)
